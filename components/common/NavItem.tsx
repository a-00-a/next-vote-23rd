...
export default function NavItem({
  ...
  if (isMobile) {
    const mobileBaseClass =
      'w-full flex items-center px-4 py-4 text-lg rounded-xl transition-colors duration-200';

    if (disabled) {
      return (
        <div className={`${mobileBaseClass} text-gray-400 cursor-not-allowed`}>
          {label}
        </div>
      );
    }
  ...
  const desktopBaseClass =
    'relative px-2 py-1 text-base font-medium transition-all duration-300 ease-out inline-block';

  if (disabled) {
    return (
      <div className={`${desktopBaseClass} text-gray-400 cursor-not-allowed`}>
        {label}
      </div>
    );
  }
